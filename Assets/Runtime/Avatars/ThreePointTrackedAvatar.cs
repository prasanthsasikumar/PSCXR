...
namespace Ubiq.Avatars
{
    ...
    public class ThreePointTrackedAvatar : MonoBehaviour, INetworkComponent
    {
        ...
        public FloatUpdateEvent OnHeartRateUpdate, OnCognitiveLoadUpdate, OnAttentionUpdate;
        ...
        private struct State
        {
            ...
            public float heartRate, cognitive_load, attention;
        }
        ...
        private void Update ()
        {
            if(avatar.IsLocal)
            {
                ...
                state[0].heartRate = GetFloatHintNode(AvatarHints.NodeFloat.HearRate);
                state[0].cognitive_load = GetFloatHintNode(AvatarHints.NodeFloat.Cognitive_load);
                state[0].attention = GetFloatHintNode(AvatarHints.NodeFloat.Attention);
        ...
        private float GetFloatHintNode(AvatarHints.NodeFloat node)
        {
            if (AvatarHints.TryGet(node, out float f))
            {
                return f;
            }
        ...
        private void OnRecv ()
        {
            ...
            var heartRate = state[0].heartRate;
            var cognitive_load = state[0].cognitive_load;
            var attention = state[0].attention;
            ...
            OnCognitiveLoadUpdate.Invoke(cognitive_load);
            OnAttentionUpdate.Invoke(attention);
        }
    }
}
