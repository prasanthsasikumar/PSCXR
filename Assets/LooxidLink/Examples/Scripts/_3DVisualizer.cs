    ...
    public class _3DVisualizer : MonoBehaviour
    {
        ...
        IEnumerator DisplayData()
        {
            while (this.gameObject.activeSelf)
            {
                ...
                if (pDController) pDController.attentionRaw = (float)attention.value;
                if (pDController) pDController.relaxation = (float)relaxation.value;
        ...
        public void SetFeautreIndexTab(EEGSensorID sensorID)
        {
            ...
            if (FeatureChannelTab != null)
            {
                for (int i = 0; i < FeatureChannelTab.Length; i++)
                {
                    ...
                    FeatureChannelTab[i].GetComponent<Image>().color = ((EEGSensorID)i == SelectFeatureSensorID) ? ConnectedColor : new Color(0.098f, 0.102f, 0.110f, 1.0f);
            }
            }
        }
